...
│  README.md
│  
├─avro // Kafka权威指南 > 3.5.2使用avro序列化
│      AvroCustomerSerializer.java
│      Customer.java
│      Main.java
│      
├─customerSerializer // Kafka权威指南 > 3.5.1 自定义序列化器
│      Customer.java
│      CustomerSerializer.java
│      Main.java
│      
└─simpleProducer // Kafka权威指南 > 3.3 发送消息到Kafka
        Main.java
````
...
在运行ch3中代码之前，需要先安装zookeeper和kafka broker。我这里给出一个可行的基于docker的安装方案。它可以在一个装有docker的阿里云机器中成功安装zookeeper和kafka，并且可以通过公网地址访问到。按照步骤执行命令即可。
